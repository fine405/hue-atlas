...
function InterfacePreview({ palette }: { palette: Palette }) {
  ...
  return <section className="interface-preview" style={theme} aria-label="配色组件预览">
    ...
    <div className="preview-body"><div className="preview-heading"><span className="preview-logo"><Leaf size={20} />slow studio</span><Badge variant="secondary">灵感空间</Badge></div>
      ...
      <div className="preview-workspace"><div><p className="preview-section-label">TODAY'S LITTLE WINS</p><h3>今天的小计划</h3><button className="task-check" onClick={() => setCompleted((value) => !value)} aria-pressed={completed}>{completed ? <Check size={19} /> : <Circle size={19} />}<span>{completed ? '完成啦，做得不错！' : '为新项目收集一些灵感'}</span></button><span className="preview-helper">点一下，试试选中状态。</span></div>
