...
export function Gallery() {
  return <><Header /><main id="main" className="page-container">
    <section className="hero" aria-labelledby="hero-title">
      <div className="hero-copy"><p className="eyebrow"><span />A LITTLE COLOR, A LOT OF POSSIBILITY</p><h1 id="hero-title">好设计，从一组<br /><em>好颜色</em>开始。</h1><p className="hero-description">精心搭配的色系，为你的下一个作品找到灵感。<br />从自然到日常，发现颜色之间恰好的默契。</p><div className="hero-actions"><Button asChild size="lg"><a href="#palettes">探索色系<ArrowDown data-icon="inline-end" /></a></Button><span>用色彩，让想法发生。</span></div><div className="hero-meta"><span><b>{palettes.length}</b> 预置色系</span><span><b>{palettes.reduce((total, palette) => total + palette.colors.length, 0)}</b> 灵感色彩</span><span><span className="tiny-dot" />自由使用</span></div></div>
    ...
    <section id="palettes" className="collection" aria-labelledby="collection-title"><div className="section-heading"><div><p className="eyebrow">THE PALETTE COLLECTION</p><h2 id="collection-title">每一组，都有自己的性格。<Badge variant="outline">{palettes.length} 色系</Badge></h2></div><p><span className="tiny-squares" aria-hidden="true">▦</span>点击色块复制 · 点击封面预览</p></div><div className="palette-grid">{palettes.map((palette, index) => <PaletteCard palette={palette} index={index} key={palette.id} />)}</div></section>
