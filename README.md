...
默认画面保留原始构图和纹理；详情页对亮度边缘做有限度锐化（每通道最多调整 10/255），再按屏幕像素密度绘制。自定义配色使用 Canvas 2D 的软颜色蒙版与 RGB 差值映射保留笔触、摄影明暗和细节；几何图案保护黑白线条。它是基于原图的换色，不是可独立编辑物体或笔触的矢量/3D 重建。色卡和效果图本身并非逐像素相同颜色，修改某一色值会影响画面中接近该色值的区域。

宝石色系来自 [東丁设计的抖音图文](https://www.douyin.com/note/7353602417352641844)（2024-04-03，用户提供的视觉参考）：海蓝宝、蓝萤石、摩根石、橄榄石、重晶石、迪奥普塔兹、蓝石英、绿碧玺。每套四个 HEX 色值与排列顺序按原图录入。宝石画面根据用户选择改为原图纹理 + Canvas 换色，保留照片中的天然断面、裂纹和明暗，不再使用几何 SVG 插画。八张源图均从 (180, 370) 裁切到 (900, 980)，保留 720 × 610 原始裁切像素，置于 1008 × 616 白色画布的 (140, 3) 位置，保存为无损 WebP。移除了旧版 519 × 440 缩图步骤；自定义颜色映射会影响接近色值的区域，白色背景和高光受到保护。这是二维照片换色，不提供三维旋转或物理折射模拟。原始图像权利归各自权利人。

## 大图清晰度

详情图与已调色封面使用屏幕像素密度自适应 Canvas，高质量插值，并监听窗口尺寸、浏览器缩放与屏幕密度变化。Canvas 最多绘制到 3240 像素宽，避免大幅缩放耗尽内存。轻度亮度锐化仅在图片加载时计算一次，调色复用结果；静态封面直接显示无损源素材。高清屏适配与锐化改善显示，不会恢复源图中不存在的细节。经典参考裁切仍为 1080 像素宽；进一步提升真实细节需要更高分辨率来源。
