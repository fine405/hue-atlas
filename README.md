...
- 单色 / 整组复制；经典画面使用 Canvas 实时换色，宝石使用可换色的 SVG 矿物插画；支持原生取色器、HEX 输入、原色对比与恢复。
...
- `src/components/palette-artwork.tsx`：按色系选择画面；经典色系使用原图纹理上的 Canvas 颜色映射，首页和详情共用。
- `src/components/gemstone-artwork.tsx`：八种宝石的 SVG 晶体、矿物底座、切面、高光与裂纹。
...
`public/artwork/*.webp` 是参考图下方效果画面的裁切，用于本次参考还原。来源为 1080 × 1620 图文：蒙德里安从 y=966、孟菲斯从 y=960、洛可可/马卡龙/敦煌从 y=961、莫兰迪从 y=956 裁切到底部。原图与艺术作品的权利归各自权利人，未作开源授权声明。
...
宝石色系来自 [東丁设计的抖音图文](https://www.douyin.com/note/7353602417352641844)（2024-04-03，用户提供的视觉参考）：海蓝宝、蓝萤石、摩根石、橄榄石、重晶石、迪奥普塔兹、蓝石英、绿碧玺。每套四个 HEX 色值与排列顺序按原图录入。宝石画面由 SVG 代码独立绘制，使用确定性的晶体几何、渐变、裂纹与纹理，未嵌入参考照片；造型参考原矿物，呈现为风格化插画，保留实时换色能力，不是照片级重建。
